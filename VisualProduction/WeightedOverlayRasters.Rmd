...
# URBAN PROBABILITY
url <- "https://github.com/HotspotStoplight/HotspotStoplight/raw/main/UrbanExpansion/data/landcover_probmaps2023.tif"
temp <- tempfile(fileext = ".tif")
download.file(url, temp, method = "auto")
...
# Assuming UrbanProbability is already loaded as a multi-band raster
numBands <- nlayers(UrbanProbability)
print(paste("Number of bands:", numBands))

# Loop through each band and plot
for (i in 1:numBands) {
  # Select individual band
  singleBand <- raster(UrbanProbability, layer = i)
  
  # Plot
  plot(singleBand, main = paste("Band", i))
}
...
# dist_road <- projectRaster(dist_road, crs = coordinate_system, method = "bilinear")
