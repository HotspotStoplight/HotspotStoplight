...
```
## Define Normalization Functions (MIN-MAX)
```{r}
...
```{r}
process_and_standardize_rasters <- function(raster_list, cropbox, resolution) {
  ...
  names(standardized_rasters) <- c("BII", "Flood_Probability", "Heat_Probability", "population", "UrbanProbability", "OtherProbability", "lc_2023", "lc_2050", "GDP")
  
  return(standardized_rasters)
}

raster_list <- process_and_standardize_rasters(raster_list, cropbox, resolution)
...
```{r}
plot(LCC_Probability, col=viridis(100))
```
