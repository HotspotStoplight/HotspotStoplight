...
# Function to create plots with the dark theme
plot_raster <- function(raster, values, title) {
  num_breaks <- 255
  ...
  levelplot(raster, col.regions = color_ramp(num_breaks), at = breaks, main = title,
            par.settings = list(
              ...
              background = list(col = "#6b6b6b"),  # Set background to black
              main.text = list(col = "white"),  # Set main title color to white
              col.lab = "white",  # Set axis labels to white
              col.main = "white",  # Set main title color to white
              col.sub = "white"    # Set subtitle color to white
            ))  # Ensure color key is visible on a dark background
}

# Load your raster data and calculate the minimum and maximum values for each raster.
min_max_values <- function(raster) {
  min_val <- global(raster, fun = 'min', na.rm = TRUE)[[1]]
  max_val <- global(raster, fun = 'max', na.rm = TRUE)[[1]]
...
anthro_values <- min_max_values(Anthro_x_Risk_norm)
bio_values <- min_max_values(Bio_x_Risk_norm)
urban_values <- min_max_values(UrbanProbability_norm)

# Create and arrange plots
plot1 <- plot_raster(Anthro_x_Risk_norm, anthro_values, "Anthropogenic Risk Index")
plot2 <- plot_raster(Bio_x_Risk_norm, bio_values, "Biodiversity Risk Index")
plot3 <- plot_raster(UrbanProbability_norm, urban_values, "Urban Probability - Random Forest Model, 2033")
