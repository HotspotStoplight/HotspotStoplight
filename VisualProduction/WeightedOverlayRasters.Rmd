...
UrbanExProbability <- raster(lc_2023)  # create a raster template from lc_2023

# Set values: where lc_2023 is 3, UrbanExProbability should be 0
# where lc_2023 is not 3, it should take the value from UrbanProbability
values(UrbanExProbability) <- ifelse(values(lc_2023) == 3, 0, values(UrbanProbability))
...
Hotspot_Stoplight_Expansion <- norm(UrbanExProbability - Bio_Loss_Risk - Climate_Impact_Risk)
```
...
STOPLIGHTplotRasterWithFisherBreaks(Hotspot_Stoplight_Expansion, "Integrated Development Risk Index for Urban Expansion")
...
quantile_list <- c(0.75, 0.90, 0.97)
