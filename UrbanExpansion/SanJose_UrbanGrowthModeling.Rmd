...
2.3: Population data: downloaded from GHS and joined to the fishnet by distributing population totals proportionally to each grid cell.

2.4: Highway vectors: downloaded from Open Street Map data via GeoFabrik.de, cleaned in ArcGIS Pro, and exported for proximity analysis in R.

2.5: Slope values are derived from 12.5m resolution DEM in ArcGIS Pro and imported to R for inclusion in models.

2.6: Land cover change data is used to engineer spatial lag features.

2.7: County polygons are downloaded using the `tigris` package.

2.8: Features are wrangled into a final dataset.

Data from each of these datasets is integrated into the vector fishnet to provide a comprehensive snapshot of the development process and context in and around Honiara between 2010 and 2020.
<br>

## 2.1. Land Cover Change Data

We aim to forecast the dependent variable of land cover change between 2010 and 2019. In this section, we load the land cover raster data, reclassify it, and integrate it with a vector fishnet. This allows us to parameterize spatial relationships in a regression context.
...
| ----------- | ------------------------------------------------------------------------------------- |
| 0           |  Remnant Habitat - Forest                                                             |
| 1          |   Herbaceous cover                                                                    |
| 2          |   Tree or shrub cover                                                                 |
...
- We read in `SanJose_Grid` - this is a fishnet covering the extent of our study area 
...
# aligned_raster2 <- projectRaster(raster2, raster1)
# 
# # Verify alignment
...
```{r set bounding boxes}
# Read the GeoJSON file
crop <- st_read("https://raw.githubusercontent.com/HotspotStoplight/HotspotStoplight/main/CropBoxes/CR_Crop3.geojson")
...
rectangle_sp <- as(crop, "Spatial")

# Create an empty raster with 300m resolution
raster_template <- raster(extent(crop), resolution = resolution)

# Create the fishnet
fishnet <- rasterToPolygons(raster_template, fun=NULL, dissolve=FALSE)

# Plot the fishnet
plot(fishnet, main = "Fishnet Grid", col = 'blue', border = 'black')

# If you also want to plot the original sf object for comparison
plot(st_geometry(crop), add = TRUE, col = 'red')
...
lc_change <- projectRaster(lc_change, crs = CRS("+init=epsg:3832"))
...
Now we convert all 0’s to `NA`. We apply a name to the raster with `names`. This is done to make it faster to join raster to the fishnet below. 
...
```{r, warning = FALSE, message = FALSE}
lc_2010 <- raster("https://raw.githubusercontent.com/Yingtong-Z/sprawl-forecasting/b528d725eaea5dd778c41fa644b5e6f71d6291b2/data/Cville_LC_2001.tif")

ggplot() +
  geom_raster(data=rast(lc_2010) %>% na.omit %>% filter(value > 0), 
              aes(x,y,fill=as.factor(value))) +
  scale_fill_viridis(discrete=TRUE, name ="") +
  labs(title = "Land Cover, 2010") +
  mapTheme +
  theme(legend.direction="horizontal")
```
...
| ----------- | ------------------------------------------------------------------------------------- | ----------------------------
| 10          |   Cropland, rainfed                                                                   | Farm
| 11          |   Herbaceous cover                                                                    | Other Undeveloped
| 12          |   Tree or shrub cover                                                                 | Forest
| 30          |   Mosaic cropland (>50%) / natural vegetation (Tree, shrub, herbaceous cover) (<50%)  | Farm
| 40          |   Mosaic natural vegetation (Tree, shrub, herbaceous cover) (>50%) / cropland (<50%)  | Forest
| 50          |   Tree cover, broadleaved, evergreen, closed to open (>15%)                           | Forest
| 100         |   Mosaic Tree and shrub (>50%) / herbaceous cover (<50%)                              | Forest
| 120         |   Shrubland                                                                           | Other Undeveloped
| 150         |   Sparse vegetation (tree, shrub, herbaceous cover) (<15%)                            | Other Undeveloped
...
```{r, warning = FALSE, message = FALSE}
developed <- lc_2010 == 190
forest <- lc_2010 == 12 | lc_2010 == 40 | lc_2010 == 50 | lc_2010 == 100
farm <- lc_2010 == 30 | lc_2010 == 10 
wetlands <- lc_2010 == 160 | lc_2010 == 170
otherUndeveloped <- lc_2010 == 11 | lc_2010 == 120 | lc_2010 == 150 
water <- lc_2010 == 210
...
aggregatedRasters <-
  aggregateRaster(theRasterList, SanJose_Grid) %>%
...
ggplot()+
  geom_sf(data = fishnet, 
          aes(fill = slope), color = "transparent") + mapTheme
```
