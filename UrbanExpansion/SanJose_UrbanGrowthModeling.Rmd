...
crop <- st_read("https://raw.githubusercontent.com/HotspotStoplight/HotspotStoplight/main/CropBoxes/CR_Crop5.geojson")
...
lc_2013 <- mask(lc_2013, crop)
lc_2023 <- mask(lc_2023, crop)

lc_2013 <- projectRaster(lc_2013, crs = coordinate_system)
lc_2023 <- projectRaster(lc_2023, crs = coordinate_system)

```

```{r make fishnet}
# Get extents of lc_2013
lc_2013_extent <- extent(lc_2013)

# Create an empty raster with lc_2013's extent
r <- raster(lc_2013_extent, res = resolution, crs = coordinate_system)
...
# Plot the fishnet
plot(SJ_Grid, main = "Fishnet Grid", col = 'blue')
...
```{r, warning = FALSE, message = FALSE}
theRasterList <- c("urban", "forest", "crop", "water")

aggregatedRasters <-
  aggregateRaster(theRasterList, SJ_Grid) %>%
  dplyr::select(urban,forest,crop,water) %>%
...
aggregatedRasters <- aggregatedRasters %>%
    mutate(crop = ifelse(developed == 0 & farm == 1, 1, 0)) %>%
    mutate(forest = ifelse(developed == 0 & farm == 0 & forest == 1, 1, 0)) %>%
    mutate(water = ifelse(developed == 0 & farm == 0 & forest == 0 & water == 1, 1, 0))

aggregatedRasters %>%
  gather(var,value,urban:water) %>%
  st_cast("POLYGON") %>%    #just to make sure no weird geometries slipped in
  mutate(X = xyC(.)$x,
         Y = xyC(.)$y) %>%
  ggplot() +
    geom_sf(data=SJ_Grid) +
    geom_point(aes(X,Y, colour=as.factor(value)), size =0.1) +
    facet_wrap(~var) +
    scale_colour_manual(values = palette2,
                        labels=c("Other","Land Cover"),
                        name = "") +
    labs(title = "Land Cover Types, 2013",
...
interpolate_column2000 <- function(column_name) {
  ...
  interpolated_data <-
    st_interpolate_aw(cvam_2000_census[column_name], SJ__Grid, extensive=TRUE) %>%
    as.data.frame(.) %>%
...
interpolate_column2020 <- function(column_name) {
  ...
  interpolated_data <-
    st_interpolate_aw(cvam_2020_census[column_name], SJ__Grid, extensive=TRUE) %>%
    as.data.frame(.) %>%
    rownames_to_column(var = "fishnetID") %>%
    left_join(SJ__Grid %>%
                mutate(fishnetID = as.character(fishnetID)),
...
highwayPoints <-
  ...
  as.data.frame() %>%
  st_as_sf(coords = c("x", "y"), crs = st_crs(SJ__Grid))

highwayPoints_fishnet <- 
  aggregate(highwayPoints, SJ__Grid, mean) %>%
...
studyAreaCounties <- 
  ...
  st_transform(st_crs(SJ__Grid)) %>%
  dplyr::select(NAME) %>%
  .[st_buffer(SJ__Grid,-1), , op=st_intersects] 
```
